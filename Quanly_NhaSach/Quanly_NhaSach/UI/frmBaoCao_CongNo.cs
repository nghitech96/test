...
{
    public partial class frmBaoCao_CongNo : Form
    {
        public frmBaoCao_CongNo()
        {
            InitializeComponent();
        }
        ...
        private async void LayDanhsachKhachhang()
        {
            ...
            var dskh1 = await _kh.LayDanhsachKhachHang();
        ...
        private async void btnThongke_Click(object sender, EventArgs e)
        {
            ...
            if (kq!=null)
            {
                if (thongtin.Id>0)
                {
                  kq =  kq.Where(x => x.IdKhachHang == thongtin.Id).ToList();
                }
